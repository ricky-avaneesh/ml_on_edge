...
def time_image_predictions(image_path):
    ...
    for i in range(len(layers_of_network)-1):
        ...
        # time_upload = feature map transmission from server to client
        # download_time = result transmission time
        # time_inference_server = time taken during convolutions at server
        # time_inference_client = time inference during convolutions at client
    ...
    plt.savefig("alexnet_edge_gpu.eps",format='eps', dpi=1000)
    plt.show()
